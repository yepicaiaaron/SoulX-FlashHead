...
```
┌─────────────────┐     WebRTC      ┌─────────────────────────────────────┐
│   LiveKit       │◄───Audio/Video──►│  SoulX-FlashHead Bot                │
│   Server        │                  │                                     │
│                 │                  │  ┌──────────────┐  ┌─────────────┐ │
└─────────────────┘                  │  │ LiveKit      │  │ SoulXFlash  │ │
        ▲                            │  │ Transport    │──│ HeadService │ │
        │                            │  │ (input)      │  │             │ │
        │                            │  └──────────────┘  └──────┬──────┘ │
        │                            │                           │        │
   ┌────┴────┐                       │  ┌──────────────────────┐ │        │
   │  User   │◄──────────────────────┼──│ LiveKit              │─┘        │
   │ Browser │    WebRTC Video       │  │ Transport (output)   │          │
   └─────────┘                       │  └──────────────────────┘          │
                                    └─────────────────────────────────────┘
```
...
- **Real-time audio-to-video streaming**: Audio from LiveKit is streamed into SoulX-FlashHead model
- **WebRTC output**: Generated video frames are streamed back via LiveKit
- **Streaming inference**: Chunk-based audio processing for low latency
- **Pipecat orchestration**: Modular pipeline architecture

## Files

| File | Description |
|------|-------------|
| `bot.py` | Main Pipecat orchestration script |
| `pipecat_soulx_service.py` | SoulXFlashHeadService implementation (draft) |
| `requirements_pipecat.txt` | Pipecat and LiveKit dependencies |
| `run_bot.sh` | Launch script with environment setup |
| `setup_node.sh` | Node environment setup script |
| `deploy.sh` | Deployment script for soulx-flashhead-node |
